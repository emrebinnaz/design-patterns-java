...
class BadCreature {
    ...
    // I can use all abilities of creature but if I add new skill to creature I have to rewrite "max", "sum" and "average" functions.
    // It violates Open Closed Principle so it is a bad example.
    public int max()
    {
...
class Creature implements Iterable<Integer> {
    ...
    private final int STR_INDEX = 0;

    public int getStrength() { return stats[STR_INDEX]; }
    public void setStrength(int value)
    {
        stats[STR_INDEX] = value;
    }
    ...
    public int min() {
        return IntStream.of(stats).min().getAsInt();
    }
...
public class ArrayBackedProperties {
    public static void main(String[] args) {
        ...
        final Iterator<Integer> iterator = creature.iterator();
        System.out.println(iterator.next()); // 17
